...
env = Environment(loader=FileSystemLoader(files("ohm_builder") / "templates"))
...
def test_html_slides(capsys):
    with capsys.disabled():
        with open(Path(__file__).parent / "acceptanceTestSlides.md") as file:
            content = file.read()
            with FiftyOhmHtmlSlideRenderer(question_renderer=question_stub) as renderer:
                output = renderer.render(Document(content))
                with open(Path(__file__).parent / "acceptanceTestSlides.html", "w") as output_file:
