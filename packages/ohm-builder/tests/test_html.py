...
env = Environment(loader=FileSystemLoader(files("ohm_builder") / "templates" / "html"))
...
def test_html(capsys):
    with capsys.disabled():
        with open(Path(__file__).parent / "acceptanceTest.md") as file:
            content = file.read()
            with FiftyOhmHtmlRenderer(
                ...
                with open(Path(__file__).parent / "acceptanceTest.html", "w") as output_file:
